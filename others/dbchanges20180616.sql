...
DELIMITER $$
CREATE DEFINER = CURRENT_USER TRIGGER `dc_BEFORE_INSERT` BEFORE INSERT ON `dc` FOR EACH ROW
BEGIN
...
DELIMITER $$
CREATE DEFINER = CURRENT_USER TRIGGER `dc_BEFORE_UPDATE` BEFORE UPDATE ON `dc` FOR EACH ROW
BEGIN
  	INSERT INTO history (table_name,primary_id, previous_value, blame_user)
    SELECT 'dc',old.id,diff.row_diff,NEW.blame_user FROM
...
DROP TRIGGER IF EXISTS `cdcitems_BEFORE_UPDATE`;
