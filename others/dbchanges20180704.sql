...
BEGIN
    ...
    SELECT 'grn',old.id,diff.row_diff,NEW.blame_user FROM
      (SELECT concat_ws('','{','"id":','"',old.id,'"',CASE when old.idsupplier <> new.idsupplier OR (old.idsupplier is NULL XOR new.idsupplier is NULL) then concat_ws( '',',"idsupplier":','"',old.idsupplier,'"') else '' END
                                                      ...
                                                      ,CASE when old.against <> new.against OR (old.against is NULL XOR new.against is NULL) then concat_ws( '',',"against":','"',old.against,'"') else '' END
                                                      ,CASE when old.supplier_dc_no <> new.supplier_dc_no OR (old.supplier_dc_no is NULL XOR new.supplier_dc_no is NULL) then concat_ws( '',',"supplier_dc_no":','"',old.supplier_dc_no,'"') else '' END
                                                      ,CASE when old.against_other <> new.against_other OR (old.against_other is NULL XOR new.against_other is NULL) then concat_ws( '',',"against_other":','"',old.against_other,'"') else '' END
